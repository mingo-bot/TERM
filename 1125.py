    ...
    def move_piece(self, start: str, end: str): #말 이동 함수
        ...
        if isinstance(piece, Pawn):
            if (piece.color == "white" and end_row == 0) or (piece.color == "black" and end_row == 7):
                print("프로모션할 기물을 선택하세요.")
                
                valid = {"Q", "R", "B", "N"}
                choice = None
                while choice not in valid:
                    choice = input("Queen(Q), Rook(R), Bishop(B), Knight(N) 중 하나 입력: ").strip().upper()
                    if choice not in valid:
                        print("잘못된 입력입니다. Q, R, B, N 중 하나를 입력해주세요.")


                if choice == "Q":
                    new_piece = Queen(piece.color, (end_row, end_col),
                                    "♕" if piece.color == "white" else "♛")
                elif choice == "R":
                    new_piece = Rook(piece.color, (end_row, end_col),
                                    "♖" if piece.color == "white" else "♜")
                elif choice == "B":
                    new_piece = Bishop(piece.color, (end_row, end_col),
                                    "♗" if piece.color == "white" else "♝")
                elif choice == "N":
                    new_piece = Knight(piece.color, (end_row, end_col),
                                    "♘" if piece.color == "white" else "♞")

                self.grid[end_row][end_col] = new_piece
                print(f"{piece.color} Pawn이 {new_piece.__class__.__name__}으로 승격되었습니다!")
...
class ChessUI:
    ...
    def on_click(self, event):
        col = event.x // 60
        row = event.y // 60
        if self.selected is None: # 첫 번째 클릭
            self.selected = (row, col) # 두 번째 클릭
        else:
            start = self.board.to_chess_notation(*self.selected) # 선택한 말
            end = self.board.to_chess_notation(row, col) # 목표 칸
            moved = self.board.move_piece(start, end) 
            if moved: # 이동 성공 시 체스판과 말을 다시 불러옴
                self.draw_board()
                self.draw_pieces()
